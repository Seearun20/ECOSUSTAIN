  // Initialize Lucide icons
    ...
    let products = [
      {
        id: 1,
        productCode: "ECO-TSH-001",
        name: "Organic Cotton T-Shirt",
        image: "/api/placeholder/100/100",
        recyclable: true,
        sustainabilityScore: 85,
        recyclabilityStatus: "Fully Recyclable",
        carbonFootprint: {
          rawMaterial: 2.5,
          manufacturing: 3.2,
          transportation: 1.8,
        ...
        row.innerHTML = `
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
            <div class="flex space-x-2">
              <button class="text-purple-600 hover:text-purple-900 qr-btn" data-id="${product.id}" title="Generate QR Code">
                <i data-lucide="qr-code" class="h-4 w-4"></i>
              </button>
              <button class="text-blue-600 hover:text-blue-900 edit-btn" data-id="${product.id}" title="Edit Product">
                <i data-lucide="edit" class="h-4 w-4"></i>
              </button>
              <button class="text-red-600 hover:text-red-900 delete-btn" data-id="${product.id}" title="Delete Product">
    ...
    saveProductBtn.addEventListener('click', () => {
      ...
      if (editingProduct) {
        products = products.map(p => p.id === editingProduct.id ? newProduct : p);
      } else {
